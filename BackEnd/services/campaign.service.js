...
export async function getCategories() {
  const { rows } = await readPool.query(`SELECT * FROM campaign_categories ORDER BY name`);
  return rows;
}

export async function followCampaign(campaign_id, user_id) {
  const { rows: campaign } = await readPool.query(`SELECT id FROM campaigns WHERE id = $1`, [campaign_id]);
  ...
  await writePool.query(`INSERT INTO campaign_followers (campaign_id, user_id) VALUES ($1, $2)`, [campaign_id, user_id]);
...
export async function unfollowCampaign(campaign_id, user_id) {
  ...
  await writePool.query(`DELETE FROM campaign_followers WHERE campaign_id = $1 AND user_id = $2`, [campaign_id, user_id]);
  return { message: "Campaign unfollowed successfully" };
}

export async function uploadMedia(campaign_id, user_id, file) {
  const { rows: campaign } = await readPool.query(`SELECT id, creator_id FROM campaigns WHERE id = $1`, [campaign_id]);
  if (!campaign[0]) throw new ResponseError("Campaign not found", 404);
...
export async function deleteMedia(campaign_id, media_id, user_id) {
  const { rows: campaign } = await readPool.query(`SELECT creator_id FROM campaigns WHERE id = $1`, [campaign_id]);
  if (!campaign[0]) throw new ResponseError("Campaign not found", 404);
  if (campaign[0].creator_id !== user_id) throw new ResponseError("You are not the owner of this campaign", 403);
  const { rows: media } = await readPool.query(`SELECT * FROM campaign_media WHERE id = $1 AND campaign_id = $2`, [media_id, campaign_id]);
