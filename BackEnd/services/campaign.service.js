...
export async function invalidateCampaignListCache() {
  try {
    let cursor = "0";
    do {
    ...
    console.warn(
      "[Cache] Failed to invalidate campaign list cache:",
      error.message,
    );
  }
}
...
async function invalidateCampaignDetailCache(campaignId) {
  try {
    ...
    console.warn(
      "[Cache] Failed to invalidate campaign detail cache:",
      error.message,
    );
  }
}
...
export async function getAllCampaigns({
  ...
  SELECT 
    ...
    (
      SELECT url FROM campaign_media 
      WHERE campaign_id = c.id AND media_type = 'image' 
...
`;
  ...
  if (creator_id) {
    ...
    query += ` AND c.creator_id = $${params.length}`;
  }
  ...
  try {
    await redisConnection.set(
      ...
      JSON.stringify(rows),
      "EX",
      CAMPAIGN_LIST_TTL_SECONDS,
    );
  } catch (error) {
    console.warn(
      "[Cache] Failed to write campaigns list cache:",
      error.message,
    );
  }
...
export async function getCampaignById(id) {
  ...
  try {
    ...
    console.warn(
      "[Cache] Failed to read campaign detail cache:",
      error.message,
    );
  }
  ...
  try {
    await redisConnection.set(
      ...
      "EX",
      CAMPAIGN_DETAIL_TTL_SECONDS,
    );
  } catch (error) {
    console.warn(
      "[Cache] Failed to write campaign detail cache:",
      error.message,
    );
  }
...
export async function updateCampaign(
  ...
  if (owned[0].status === "PendingApproval") {
    throw new ResponseError(
      "Cannot edit campaign while it is awaiting admin approval",
      400,
    );
  }
  const { rows } = await writePool.query(
...
export async function deleteCampaign(id, creator_id) {
  ...
  if (!owned[0])
    throw new ResponseError("You are not the owner of this campaign", 403);
  await writePool.query(`DELETE FROM campaigns WHERE id = $1`, [id]);
  await invalidateCampaignDetailCache(id);
  await invalidateCampaignListCache();
...
export async function getCategories() {
  const { rows } = await readPool.query(
    `SELECT * FROM campaign_categories ORDER BY name`,
  );
  return rows;
}

export async function followCampaign(campaign_id, user_id) {
  const { rows: campaign } = await readPool.query(
    `SELECT id FROM campaigns WHERE id = $1`,
    [campaign_id],
  );
  ...
  if (existing[0])
    throw new ResponseError("You are already following this campaign", 409);
...
export async function unfollowCampaign(campaign_id, user_id) {
  ...
  if (!existing[0])
    throw new ResponseError("You are not following this campaign", 404);
  await writePool.query(
...
export async function uploadMedia(campaign_id, user_id, file) {
  ...
  if (!campaign[0]) throw new ResponseError("Campaign not found", 404);
  if (campaign[0].creator_id !== user_id)
    throw new ResponseError("You are not the owner of this campaign", 403);
...
export async function deleteMedia(campaign_id, media_id, user_id) {
  ...
  if (campaign[0].creator_id !== user_id)
    throw new ResponseError("You are not the owner of this campaign", 403);
  const { rows: media } = await readPool.query(
    `SELECT * FROM campaign_media WHERE id = $1 AND campaign_id = $2`,
  ...
  await writePool.query(`DELETE FROM campaign_media WHERE id = $1`, [media_id]);
